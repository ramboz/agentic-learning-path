...
- [**AI-First Development Guidelines**](https://github.com/adobe/mysticat-ai-native-guidelines). The team's operating playbook, in more depth than a module carries. For how the team packages and distributes skills, see its skills and skill-distribution sections. This module is the trigger model; the guidelines are how the team ships skills across projects.
...
- [**jig (separate side project)**](https://github.com/ramboz/jig). Optional further reading if you want another battle-tested reference. Counterpoint to maximalist skill packs: a deliberately small skill pack (≈10 skills, 3 sub-agents named `architect` / `implementer` / `reviewer`) with an explicit "extension points" pattern where baseline skills like `/jig:pr-review` defer via category-based description matching to richer user-installed skills when both are present. The README's "Design philosophy" and "Extension points" sections are the load-bearing read.
- [**Experience Success skills**](https://github.com/adobe/experience-success-skills) (private repo, Adobe-internal). A real, shipped skill pack: what packaged, distributed skills look like in practice, beyond the two single-file skills the lab builds.
