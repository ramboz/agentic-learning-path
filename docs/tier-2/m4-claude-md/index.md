...
- [**AI-First Development Guidelines**](https://github.com/adobe/mysticat-ai-native-guidelines). The team's operating playbook, in more depth than a module carries. For the production version of CLAUDE.md, see its workspace and project CLAUDE.md examples and Claude Code configuration notes. This module is the why; the guidelines are how the team sets one up.
- [**"claude_code_docs_map.md" (Willison)**](https://simonwillison.net/2025/Oct/24/claude-code-docs-map/). Worked example of a markdown map file as on-demand context: the model reaches for it only when it needs to. Pattern transfers directly to the file-based state principle this module argues for.
...
- [**jig (separate side project)**](https://github.com/ramboz/jig). Optional further reading if you want another battle-tested reference. Its CLAUDE.md leads with a "Hot Cache" section of high-frequency project state loaded every session, plus a memory layer split across hot cache, deep storage, and an inbox for parked ideas. Same file-based-state principle, several months into active work.
